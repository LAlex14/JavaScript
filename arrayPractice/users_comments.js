...
const whoFirstComm = users.find(user => user.id === comments[0].userId);
// console.log(whoFirstComm.firstName + ' ' + whoFirstComm.lastName);

// 3) Which user commented 'OK great thanks'?
const ex3 = users.find(user => user.id === (comments.find(comment => comment.text === 'OK great thanks').userId))
// console.log(ex3);

// 4) Add the user's first and last name to each comment in the comments array
const ex4 = comments.map(comment => {
    let currUser = users.find(user => user.id === comment.userId);
    return { ...comment, text: `${currUser.firstName} ${currUser.lastName}: ${comment.text}` }
});
// console.log(ex4);

// 5) Get a list of the users who haven't commented
const ex5 = users.filter(user => comments.find(comment => comment.userId === user.id) === undefined);
console.log(ex5);